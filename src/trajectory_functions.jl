...
function create_callback_generator(trajectory_length=40, timestep=0.2, R = Diagonal([0.1, 0.5]), max_vel=10.0)
    ...
    X¹, X², X³, r¹, r², r³, track_center, track_rad, lane_width, Z = let
        @variables(X¹[1:4], X²[1:4], X³[1:4], r¹, r², r³, track_center[1:2], track_rad, lane_width, Z[1:6*trajectory_length]) .|> Symbolics.scalarize
    end
    ...
    for k in 1:trajectory_length
        ...
        append!(constraints_val, curved_lane_constraint(states[k], track_center, track_rad-lane_width/2+r¹))
        append!(constraints_val, curved_lane_constraint(states[k], track_center, track_rad+lane_width/2-r¹))
        append!(constraints_lb, 0.0)
        append!(constraints_ub, Inf)
        append!(constraints_lb, -Inf)
        append!(constraints_ub, 0)
    ...
    full_cost_fn = let
        cost_fn = Symbolics.build_function(cost_val, [Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width]; expression)
        (Z, X¹, X², X³, r¹, r², r³, track_center, track_rad, lane_width) -> cost_fn([Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width])
    end

    full_cost_grad_fn = let
        cost_grad_fn! = Symbolics.build_function(cost_grad, [Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width]; expression)[2]
        (grad, Z, X¹, X², X³, r¹, r², r³, track_center, track_rad, lane_width) -> cost_grad_fn!(grad, [Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width])
    end

    full_constraint_fn = let
        constraint_fn! = Symbolics.build_function(constraints_val, [Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width]; expression)[2]
        (cons, Z, X¹, X², X³, r¹, r², r³, track_center, track_rad, lane_width) -> constraint_fn!(cons, [Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width])
    end

    full_constraint_jac_vals_fn = let
        constraint_jac_vals_fn! = Symbolics.build_function(jac_vals, [Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width]; expression)[2]
        (vals, Z, X¹, X², X³, r¹, r², r³, track_center, track_rad, lane_width) -> constraint_jac_vals_fn!(vals, [Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width])
    end

    full_hess_vals_fn = let
        hess_vals_fn! = Symbolics.build_function(hess_vals, [Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width; λ; cost_scaling]; expression)[2]
        (vals, Z, X¹, X², X³, r¹, r², r³, track_center, track_rad, lane_width, λ, cost_scaling) -> hess_vals_fn!(vals, [Z; X¹; X²; X³; r¹; r²; r³; track_center; track_rad; lane_width; λ; cost_scaling])
    end
...
function curved_lane_constraint(X, c, r)
    (X[1:2]-c)'*(X[1:2]-c) - r*r
end
